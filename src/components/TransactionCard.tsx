...
//       try {
//         const messages = await apiFeedback.getTransactionChats(transaction.id); // Replace with your real API
...
//               <AlertTriangle
//                 className={`h-5 w-5 ${
//                   transaction.score > 0.7
//                     ? "text-fraud-high"
//                     : transaction.score > 0.4
//                     ? "text-fraud-medium"
//                     : "text-fraud-low"
//                 }`}
//               />
...
//         >
//           <FileText className="h-4 w-4 mr-1" /> {showChat ? "Hide Chat" : "Chat"}
//         </Button>

//         {showChat && (
//           <div className="w-full bg-muted/30 rounded p-2 border text-sm space-y-2 max-h-64 overflow-y-auto">
//             {isLoadingChat ? (
//               <p>Loading chat...</p>
//             ) : chatMessages.length ? (
//               chatMessages.map((msg, idx) => (
//                 <div key={idx} className="bg-background p-2 rounded shadow-sm border">
//                   {msg}
//                 </div>
//               ))
//             ) : (
...
//       setIsLoadingChat(true);
//       try {
//         const messages = await apiFeedback.getTransactionChats(transaction.id);
//         setChatMessages(messages);
//       } catch (error) {
//         console.error("Failed to fetch chat messages:", error);
//         toast.error("Could not load chat.");
  ...
  const handleShowChat = async () => {
    ...
    if (nextState) {
      ...
      try {
        ...
        const feedbackMessages = Array.isArray(messages)
          ? messages.map((msg) => msg.feedback).filter(Boolean)
          : [];

        setChatMessages(feedbackMessages);
      } catch (error) {
        console.error("Failed to fetch chat messages:", error);
        toast.error("Could not load chat.");
      } finally {
