...
//     score: 0.85,
//     reason:
//       "Transaction T123456 was flagged because the amount of ₹15,000 exceeds typical daily averages by over 300% for this account. The transaction also occurred from an unusual IP address not previously associated with this account.",
//   },
//   {
//     id: "T123457",
//     timestamp: "2025-04-07T10:23:45",
...
//     score: 0.62,
//     reason:
//       "Transaction T123457 was flagged due to unusual merchant category. This account typically performs transactions in retail and grocery, but this payment was made to a high-risk merchant category in a foreign jurisdiction.",
//   },
//   {
//     id: "T123458",
//     timestamp: "2025-04-07T14:56:12",
...
//     score: 0.35,
//     reason:
//       "Transaction T123458 triggered a low-level alert due to being slightly outside normal spending patterns. The transaction amount is higher than average for this merchant type, but remains within reasonable bounds for the account history.",
//   },
//   {
//     id: "T123459",
//     timestamp: "2025-04-06T18:34:22",
...
//     score: 0.78,
//     reason:
//       "Transaction T123459 was flagged as potentially fraudulent because this ATM withdrawal occurred in a location over 500km from the account holder's typical transaction area. Additionally, there were 3 failed PIN attempts before this successful transaction.",
//   },
//   {
//     id: "T123460",
//     timestamp: "2025-04-06T11:45:33",
...
//     score: 0.52,
//     reason:
//       "Transaction T123460 raised moderate concern due to unusual timing. This transaction occurred at 11:45 PM, while the account holder typically makes purchases between 8 AM and 8 PM. The merchant category itself is not unusual for this customer.",
//   },
// ];
...
//   getTransactionChats: async (transactionId: string): Promise<string[]> => {
//     try {
//       return await fetchAPI<string[]>(`/feedback/transaction/${transactionId}`);
//     } catch (error) {
//       console.warn("Failed to fetch chat messages, returning mock:", error);
//       return [
//         `Why was transaction ${transactionId} flagged?`,
//         "The transaction was flagged due to unusual activity on this account.",
//       ];
//     }
//   },
// };
...
// Prevent duplicate submissions by storing active transaction feedback requests
const activeFeedbackRequests = new Set<string>();
...
export const apiFeedback = {
  submitTransactionFeedback: async (
    ...
    // Prevent multiple submissions for the same transaction
    if (activeFeedbackRequests.has(transactionId)) {
      console.warn("Feedback already being submitted for this transaction:", transactionId);
      return { success: false, message: "Feedback already submitted or in progress" };
    }

    activeFeedbackRequests.add(transactionId);

    try {
      const result = await fetchAPI<{ success: boolean; message?: string }>("/feedback/transaction", {
        method: "POST",
        body: JSON.stringify({
          transaction_id: transactionId,
          is_correct: isCorrect,
      ...
      console.warn("Failed to submit feedback, simulating offline success:", error);
      return { success: true, message: "Feedback submitted (offline mode)" };
    } finally {
      activeFeedbackRequests.delete(transactionId);
    }
  },
  ...
  submitSystemFeedback: async (category: string, details: string) => {
    try {
      ...
      console.warn("Failed to submit system feedback, simulating success:", error);
      return { success: true, message: "System feedback submitted (offline mode)" };
    }
  },
